...
Play.prototype = {
	init: function(map) {
		// for onCollisionGotoMap property for portals, WIP
		this.map = 'map';
		if (map) {
			this.map = map;
		}
	...
	update: function() {
		...
		_.each(this.map.sprites, function(sprite) {
			if (this.map.collide(sprite, this.map.player)) {
				...
				if (sprite.onCollisionGotoState) {
					// now remember at the moment your map is based on in the hash side of the window.location uri.
					// This mean you have the power of onCollisionGotoState to jump between states, but keep the same map.
					var gotoObj = {};
					this.game.state.start(sprite.onCollisionGotoState, true, false, gotoObj);
				...
				if (sprite.onCollisionText && !sprite.text) {
					var t = this.map.createText(sprite.x, sprite.y, sprite.onCollisionText);
					sprite.text = t;
...
// Export our Play State Constructor to the Browsify and Node peeps.
module.exports = Play;
